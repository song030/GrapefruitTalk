...
class Server(object):
    ...
    __BUFFER = 4000 # 버퍼
    __PORT = 1121  # 포트
    __HOST = '10.10.20.103' # socket.gethostbyname(socket.gethostname()) #호스트 ip
    __ADDR = (__HOST, __PORT) # 호스트 ip와 port 주소를 튜플로 저장
    __FORMAT = "utf-8" # 포메팅은 utf-8로
    __connected_clients = {} # 연결된 클라이언트의 key/value 쌍을 매핑하는 딕셔너리입니다.
    __connected_usernames = [] # 모든 클라이언트에게 표시하기 위해 사용자 이름을 저장하는 리스트입니다.
    ...
    def start(self):
        ...
        try:
            self.__server.listen() # 연결 요청 대기

            while True:
                # accept(): 서버의 연결 요청 대기. 클라이언트가 연결 요청 할 때까지 대기한다.
                sock, addr = self.__server.accept() # 소켓 객체, 주소를 튜플 형식으로 반환받는다.
                ...
                if self.__validate_username(username): # True값 리턴되면
                    sock.send("Valid".encode(self.__FORMAT)) # utf-8값으로 인코딩해서 값을 보내줌 '유효함'
                    new_user = dict(user = username, address = addr, client = sock) #새로운 딕셔너리 값을 만듭니다.
                    self.__connected_clients[username] = new_user #딕셔너리 내에 새로운 key/value 쌍을 만듭니다.
                    self.__connected_usernames.append(username) # 사용자 이름 리스트에 유저 이름을 추가한다.
                    self.__broadcast(username, "[*] " + username + "님이 입장하셨습니다. ") #새로운 참가자가 들어왔다는 메시지를 모든 현재 연결된 클라이언트에게 보냅니다.
                    self.__list_of_users() # 현재 접속 유저들에게 접속 유저 모두 전송
                    ...
                    sock.send("Invalid".encode(self.__FORMAT)) # 유효하지 않음 신호전달
    ...
    def __validate_username(self, username):
        ...
        if self.__connected_clients.get(username) == None: # 주어진 사용자 이름이 연결된 클라이언트에게 매핑되어있지 않은 경우
    ...
    def __list_of_users(self):
        ...
        user_list = pickle.dumps(self.__connected_usernames) # 연결된 유저들을 모두 pickle로 직렬화 하여 리스트에 담는다.
        for _, values in self.__connected_clients.items(): # 연결된 클라이언트들의 username : value 값을 for문으로 돌려서
    ...
    def __broadcast(self, username, message):
        """현재 연결된 모든 클라이언트에게 메시지를 보냅니다.
           ...
           :rtype: bool
        """
        for key, values in self.__connected_clients.items(): # 연결된 유저들을 for문으로 돌려서 key값이 user_name과 같다면(접속한 클라이언트라면)
            if key == username:
                continue  # 현재 클라이언트 배제
            else:
                values['client'].send(message.encode(self.__FORMAT)) # 메세지를 포메팅해서 해독한 후에 클라이언트들에게 보내줌

        # for user, data in self.__connected_clients.items():
        #     client = data.get("client")
        #     try:
        #         client.send((username + ": " + message).encode(self.__FORMAT))
        #     except socket.error as ex:
        ...
        while connected:
            try:
                ...
                if message: # 메세지가 빈 값이 아니라면
                    if message == 'quit':
                        connected = False
                        # self.__broadcast(username, "[*] " + username + "님이 나가셨습니다. ")
                        # self.__connected_usernames.remove(username)
                        continue
                    else:
                        message_date = datetime.datetime.now()
                        self.__broadcast(username, ">" + username + ": " + message + " - " + message_date.strftime(
                            " %x %I:%M %p"))

                # else:
                #     self.__broadcast(username, message)
            # except socket.error as ex:
            #     print("클라이언트 연결 해제.")
            #     connected = False
    ...
    def __remove_user(self, username):
        ...
        if len(self.__connected_usernames) != 0:  # 만약 마지막 사람이 제거된다면, 이 메세지를 출력하지 않는다.
